            ...
            for i in range(16):
                for j in range(i, 16):
                    ...
                    if(dist < threshold) and (x != 0 or y != 0 or z != 0 or (i-j)%4 != 0):
                        ...
                        ## sp sigma bonds
                        # elif(start_cell[i].type=='px' and target_cell[i].type=='s'):
                        #     dir_cos = start_cell.findDirCos();
                        #     if (i, j, -x, -y, -z, 'px', 's') not in dist_dict_sp:
                        #         # add distance and direction cosion to the dict
                        #         dist_dict_sp[(i, j, x, y, z,'px','s')] = (dist, dir_cos[0])
                        # elif(start_cell[i].type=='py' and target_cell[i].type=='s'):
                        #     dir_cos = start_cell.findDirCos();
                        #     if (i, j, -x, -y, -z, 'py', 's') not in dist_dict_sp:
                        #         # add distance and direction cosion to the dict
                        #         dist_dict_sp[(i, j, x, y, z,'py','s')] = (dist, dir_cos[1])
                        # elif(start_cell[i].type=='pz' and target_cell[i].type=='s'):
                        #     dir_cos = start_cell.findDirCos();
                        #     if (i, j, -x, -y, -z, 'pz', 's') not in dist_dict_sp:
                        #         # add distance and direction cosion to the dict
                        #         dist_dict_sp[(i, j, x, y, z,'pz','s')] = (dist, dir_cos[2])
...
# dist_dict_ss_sorted = sorted(dist_dict_ss.values())
# dist_dict_sp_sorted = sorted(dist_dict_ss.values())
# dist_dict_ss_sorted = sorted(dist_dict_ss.values())
